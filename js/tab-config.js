...
/* ── Auto-connect al cargar la página ─────────────
   Si ya hay credenciales guardadas (cualquier dispositivo),
   sincroniza automáticamente con Supabase sin que el usuario
   tenga que abrir la pestaña Config ni hacer clic en nada.  */
document.addEventListener('DOMContentLoaded', function(){
  ...
  if (u && k) {
    SB_URL = u;
    SB_KEY = k;
    /* Pull silencioso: trae los datos más recientes de Supabase.
       setConnStatus() no hace nada si el panel Config no está abierto. */
    pullFromSupabase();
  }
});
